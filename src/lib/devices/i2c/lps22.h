...
constexpr std::chrono::milliseconds kLps22DefaultMeasurementInterval(2000);  /* The number of msecs that a reading is good */
/*
 * Fixed address. could be 0x55 you have to check the model from the data sheet
 */
constexpr uint8_t kLps22hbI2cAddress =  0x5D; // or 0x5C, See data sheet section 7.2.1.
...
constexpr int kLps22hbPressureHpaFactor = 4096;  // convert measured value to hPa
constexpr int kLps22hbTemperatureFactor = 100;
constexpr uint8_t kLps22hbMaxRegistersTransferred = 5;  // The maximum number of registers in one transfer
...
class Lps22 : public TemperatureInterface, public BarometricPressureInterface {
 ...
 private:
   ...
   std::chrono::milliseconds measurement_interval_ = kLps22DefaultMeasurementInterval;  // Interval between measurements
   ...
   /*
     ...
     */
   const uint8_t default_ctrl_reg_2_ = kLps22hbCtrlReg2IfAddIncMask; // So we can do multiple register reads
